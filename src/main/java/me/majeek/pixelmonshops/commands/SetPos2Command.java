        ...
        if(src instanceof Player) {
            ...
            if(array == null) {
                ...
                if(array[0] == null) {
                    ...
                    Vector3d offset = new Vector3d(position.getFloorX() - array[0].getFloorX(), position.getFloorY() - array[0].getFloorY(), position.getFloorZ() - array[0].getFloorZ());
                    double area = (offset.getFloorX() + 1) * (offset.getFloorY() + 1) * (offset.getFloorZ() + 1);

                    if(area > 1000 || area < -1000) {
                        try {
                            ConfigurationNode node = PixelmonShops.getInstance().getMessagesConfig().getLoader().load();
                            Text text = node.getNode("setPos2", "exceedAreaLimit").getValue(TypeToken.of(Text.class));
            ...
            try {
                ConfigurationNode node = PixelmonShops.getInstance().getMessagesConfig().getLoader().load();
                Text text = node.getNode("setPos2", "positionSet").getValue(TypeToken.of(Text.class));
